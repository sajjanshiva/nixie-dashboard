...
router.get("/", async (req, res) => {
  ...
  let q = supabaseAdmin.from("holidays").select("*").order("date");
...
router.post("/", async (req, res) => {
  ...
  const { data, error } = await supabaseAdmin
    .from("holidays")
    .upsert({ date, name, source: "custom" }, { onConflict: "date" })
    .select()
    .single();
  if (error) return res.status(500).json({ message: error.message });
  res.json(data);
...
router.delete("/:date", async (req, res) => {
  ...
  const { error } = await supabaseAdmin.from("holidays").delete().eq("date", req.params.date);
  if (error) return res.status(500).json({ message: error.message });
  res.json({ ok: true });
});
