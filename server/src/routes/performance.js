...
async function computePerformance(staffId, from, to) {
  ...
  const [attRows, holidayRows, leaveRows, taskRows, profileRows] = await Promise.all([
    safeQuery(supabaseAdmin.from("attendance").select("*").eq("staff_id", staffId).gte("date", from).lte("date", to).order("check_in", { ascending: true })),
    safeQuery(supabaseAdmin.from("holidays").select("date, name").gte("date", from).lte("date", to)),
    safeQuery(supabaseAdmin.from("leaves").select("date_from, date_to, status").eq("staff_id", staffId).eq("status", "approved")),
    safeQuery(supabaseAdmin.from("tasks").select("*").eq("assignee_id", staffId)),
    safeQuery(supabaseAdmin.from("profiles").select("activated_at").eq("id", staffId).limit(1)),
  ]);
...
router.get("/summary", async (req, res) => {
  ...
  try {
    const { data: members } = await supabaseAdmin.from("profiles").select("id, name").eq("role", "staff");
